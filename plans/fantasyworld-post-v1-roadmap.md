...
截至 2026-06-18，Post-v1 Step 1-8 已进入 PR #21。
目标是让创建世界和推进回合具备真实 OpenAI-compatible LLM 结构化生成路径。
同时保留 mock fallback 以支持本地测试和 CI，并让 LLM 失败进入可恢复 job，且支持存档级模型覆盖、用量/成本可见性和生产同进程后台 worker。
